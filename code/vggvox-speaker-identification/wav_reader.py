...
def get_fft_spectrum(filename, buckets):
	...
	fft_norm = normalize_frames(fft.T)  #TODO may remove variance normalization to see other poorer results
...
def save_fft_spectrum(filename, D):
	# print('saving')
	plt.figure(figsize=(6, 4))
	# plt.subplot(1, 2, 1)
	librosa.display.specshow(D, y_axis='linear',cmap='magma')
	plt.colorbar(format='%+2.0f dB')
	# plt.title('Constant-Q power spectrogram (Hz)')
	plt.title('Linear-frequency power spectrogram')

	# plt.subplot(1, 2, 2)
	# librosa.display.specshow(D, y_axis='log',cmap='magma')
	# plt.colorbar(format='%+2.0f dB')
